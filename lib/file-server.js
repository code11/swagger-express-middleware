    ...
    if (req.method === 'GET' || req.method === 'HEAD') {
      ...
      if (reqPath === configPath) {
        if (context.api) {
		  ...
		  var newContextApi = JSON.parse(JSON.stringify(context.api))
		  var pathKeys = Object.keys(newContextApi.paths)
		  for (var i=0; i< pathKeys.length; i++ )
		  {
			  var nKey = pathKeys[i].replace(/\/\/\?\.\*/, "/")
			 newContextApi.paths[nKey] = newContextApi.paths[pathKeys[i]]
			  delete newContextApi.paths[pathKeys[i]]
		  }

          res.json(newContextApi);
        }
        else {
