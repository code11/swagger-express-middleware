    ...
    if (req.method === 'GET' || req.method === 'HEAD') {
      ...
      if (reqPath === configPath) {
        if (context.api) {
		  ...
		  // ADDED FOR FORK. ENABLES WILDCARD NAMESPACING FOR SWAGGER API/DOCS

		  var pathKeys = Object.keys(context.api.paths)
		  for (var i=0; i< pathKeys.length; i++ )
		  {
			  var nKey = pathKeys[i].replace(/\/\/\?\.\*/, "/")
			  context.api.paths[nKey] = context.api.paths[pathKeys[i]]
			  delete context.api.paths[pathKeys[i]]
		  }
		  // --------------------------------------
